...
local TabScroller
do
    local scroller = TabContainer:FindFirstChild("TabScroller")
...
local T = {
    ...
    NativePageBg = Color3.new(0,0,0),
    NativePageBgT = 1,
    CustomPageBg = Color3.new(0,0,0),
    CustomPageBgT = 1,
    NativeTabBg = Color3.new(0,0,0),
    NativeTabBgT = 1,
    CustomTabBg = Color3.new(0,0,0),
    CustomTabBgT = 1,
    TabContainerBg = Color3.new(0,0,0),
    TabContainerBgT = 1,
}
CoreUi._T = T
    ...
    for _, child in next, PageInner:GetChildren() do
        if not child:IsA("UIListLayout") and not child:IsA("UIPadding") and not child:IsA("UICanvasGroup") then
            ...
            if not name:find("^CoreUi_") and not name:find("^InnerCanvasGroup") then
                ...
                local scroll = child:FindFirstChildOfClass("ScrollingFrame") or child:FindFirstChild("CoreUiScroll")
                if scroll and scroll:IsA("ScrollingFrame") then
                    NativeScrollbars[child] = {
                        scroll = scroll,
                        thickness = scroll.ScrollBarThickness,
                    }
                end
            end
        end
...
local function ShowNativePages()
    ...
    for page, data in next, NativeScrollbars do
        if data.scroll then
            data.scroll.ScrollBarThickness = data.thickness
        end
    end
end
...
local function HideNativePages()
    ...
    for page, data in next, NativeScrollbars do
        if data.scroll then
            data.scroll.ScrollBarThickness = 0
        end
    end
end
...
for _, child in next, TabScroller:GetChildren() do
    if child:IsA("TextButton") then
        ...
        AddConn(child.MouseButton1Click:Connect(function()
            ActivateNativeTab(child)
        end))
    end
end
...
local function RebuildTabSizes()
    local fixedWidth = 120
    for _, nt in next, NativeTabs do
        nt.Size = UDim2.new(0, fixedWidth, 1, 0)
        nt.AutomaticSize = Enum.AutomaticSize.None
    end
    for _, btn in next, CustomTabButtons do
        btn.Size = UDim2.new(0, fixedWidth, 1, 0)
...
local function BuildTabButton(name, title, icon)
    ...
    local btn = New("TextButton", {
        ...
        BackgroundTransparency = T.CustomTabBgT,
        BackgroundColor3 = T.CustomTabBg,
        Text = "",
...
local function BuildCustomPage(name)
    ...
    local pageFrame = New("Frame", {
        ...
        BackgroundTransparency = T.CustomPageBgT,
        BackgroundColor3 = T.CustomPageBg,
        ZIndex = 5,
        Parent = PageInner,
    }, {
        New("UIListLayout", {
...
function Elements.Slider(container, idx, config, library)
    ...
    local function UpdateSteps(pct)
        ...
        for s = 1, STEPS do
            ...
            stepFrames[s].BackgroundColor3 = active and T.Accent or Color3.fromRGB(57, 59, 61)
        ...
        OpenOverlay(function(dimmer)
            ...
            for i, opt in next, dropdown.Values do
                ...
                local item = New("TextButton", {
                    ...
                    New("UIStroke", {
                        Color = T.Border,
                        Transparency = isSelected and 0.3 or 0.1,
                        Thickness = 1,
                        ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
                    }),
                })
...
function Elements.Colorpicker(container, idx, config, library)
    ...
    local function SetValue(color, trans)
        cp.Value = color
        if trans ~= nil then cp.Transparency = trans end
    ...
    cp.SetValue = function(self, hsv, trans)
        local color = Color3.fromHSV(hsv[1], hsv[2], hsv[3])
        SetValue(color, trans or cp.Transparency)
    end

    cp.SetValueRGB = function(self, color, trans)
        SetValue(color, trans or cp.Transparency)
    end
    ...
    local function CreateDialog()
        OpenOverlay(function(dimmer)
            local panelW, panelH = 430, 330
            local absPos = displayBg.AbsolutePosition
            local panel = New("Frame", {
                Name = "CoreUiColorDialog",
            ...
            local Hue, Sat, Vib = cp.Hue, cp.Sat, cp.Vib
            local Transparency = cp.Transparency
            ...
            local function Display()
                ...
                HexInput.Text = "#" .. Color3.fromHSV(Hue, Sat, Vib):ToHex()
                RedInput.Text = GetRGB()["R"]
                GreenInput.Text = GetRGB()["G"]
                BlueInput.Text = GetRGB()["B"]
                if Config.Transparency then
                    ...
                    AlphaInput.Text = Round((1 - Transparency) * 100, 0) .. "%"
                end
            end
            ...
            local SatVibMap = New("ImageLabel", {
                ...
                RoundUICorner(4),
                New("UIStroke", { Color = T.Border, Transparency = 0.3, Thickness = 1 }),
            })
            ...
            local HueSlider = New("Frame", {
                ...
                Position = UDim2.fromOffset(195, 30),
                Parent = panel,
                ZIndex = 23,
                BackgroundTransparency = 1,
            }, {
            ...
            local HueDrag = New("ImageLabel", {
                ...
                Parent = HueSlider,
                Position = UDim2.new(0, -1, Hue, -6),
                ZIndex = 25,
                BackgroundTransparency = 1,
            })

            local TransparencySlider, TransparencyDrag, TransparencyColor, AlphaInput
            if Config.Transparency then
                local transSlider = New("Frame", {
                    Size = UDim2.fromOffset(12, 190),
                    Position = UDim2.fromOffset(215, 30),
                    Parent = panel,
                    ZIndex = 23,
                    BackgroundTransparency = 1,
                }, {
                ...
                TransparencySlider = transSlider
                TransparencyDrag = New("ImageLabel", {
                    Size = UDim2.fromOffset(14, 14),
                    Image = "http://www.roblox.com/asset/?id=12266946128",
                    Parent = transSlider,
                    Position = UDim2.new(0, -1, 1 - Transparency, -6),
                    ZIndex = 25,
                    BackgroundTransparency = 1,
                })
                TransparencyColor = transSlider:FindFirstChildOfClass("Frame")
            end
            ...
            local oldColorFrame = New("Frame", {
                ...
                Parent = panel,
                ZIndex = 23,
            }, { RoundUICorner(4), New("UIStroke", { Color = T.Border, Transparency = 0.3, Thickness = 1 }) })
            local newColorFrame = New("Frame", {
                Size = UDim2.fromOffset(88, 24),
                Position = UDim2.fromOffset(100, 200),
                BackgroundColor3 = Color3.fromHSV(Hue, Sat, Vib),
                BackgroundTransparency = Transparency,
                Parent = panel,
                ZIndex = 23,
            }, { RoundUICorner(4), New("UIStroke", { Color = T.Border, Transparency = 0.3, Thickness = 1 }) })
            local DialogDisplayFrame = newColorFrame

            local function CreateInput(initialText, posX, posY)
                local box = New("TextBox", {
            ...
            local HexInput = CreateInput("#" .. Color3.fromHSV(Hue, Sat, Vib):ToHex(), 230, 30)
            local RedInput = CreateInput(GetRGB()["R"], 230, 70)
            local GreenInput = CreateInput(GetRGB()["G"], 230, 110)
            local BlueInput = CreateInput(GetRGB()["B"], 230, 150)
            if Config.Transparency then
                AlphaInput = CreateInput(Round((1 - Transparency) * 100, 0) .. "%", 230, 190)
            end

            local function CreateLabel(text, posX, posY)
                New("TextLabel", {
            ...
            CreateLabel("Hex", 330, 30)
            CreateLabel("Red", 330, 70)
            CreateLabel("Green", 330, 110)
            CreateLabel("Blue", 330, 150)
            if Config.Transparency then
                CreateLabel("Alpha", 330, 190)
            end

            local function onFocusLost(box, parseFunc)
                box.FocusLost:Connect(function(enter)
                    if enter then
                        local ok, result = pcall(parseFunc, box.Text)
                        if ok and result ~= nil then
                            if typeof(result) == "Color3" then
                                Hue, Sat, Vib = Color3.toHSV(result)
                            elseif type(result) == "number" then
                                Transparency = math.clamp(1 - result/100, 0, 1)
                            end
                            Display()
                        else
                            Display()
            ...
            onFocusLost(HexInput, function(t)
                return Color3.fromHex(t)
            end)
            onFocusLost(RedInput, function(t)
                local rgb = GetRGB()
                return Color3.fromRGB(tonumber(t) or rgb.R, rgb.G, rgb.B)
            end)
            onFocusLost(GreenInput, function(t)
                local rgb = GetRGB()
                return Color3.fromRGB(rgb.R, tonumber(t) or rgb.G, rgb.B)
            end)
            onFocusLost(BlueInput, function(t)
                local rgb = GetRGB()
                return Color3.fromRGB(rgb.R, rgb.G, tonumber(t) or rgb.B)
            end)
            if Config.Transparency then
                onFocusLost(AlphaInput, function(t)
                    local val = tonumber(t:match("%d+")) or 0
            ...
            AddConn(UserInputService.InputChanged:Connect(function(inp)
                if inp.UserInputType == Enum.UserInputType.MouseMovement and draggingSV then
                    local ax, ay = SatVibMap.AbsolutePosition.X, SatVibMap.AbsolutePosition.Y
                    local aw, ah = SatVibMap.AbsoluteSize.X, SatVibMap.AbsoluteSize.Y
                    local mx = math.clamp(inp.Position.X - ax, 0, aw)
                    local my = math.clamp(inp.Position.Y - ay, 0, ah)
                    Sat = mx / aw
                    Vib = 1 - my / ah
                    Display()
                end
            ...
            AddConn(UserInputService.InputChanged:Connect(function(inp)
                if inp.UserInputType == Enum.UserInputType.MouseMovement and draggingHue then
                    local ay = HueSlider.AbsolutePosition.Y
                    local ah = HueSlider.AbsoluteSize.Y
                    local my = math.clamp(inp.Position.Y - ay, 0, ah)
                    Hue = my / ah
                    Display()
                end
            ...
            if Config.Transparency then
                ...
                TransparencySlider.InputBegan:Connect(function(inp)
                    if inp.UserInputType == Enum.UserInputType.MouseButton1 then
                        draggingTrans = true
                    end
                end)
                TransparencySlider.InputEnded:Connect(function(inp)
                    if inp.UserInputType == Enum.UserInputType.MouseButton1 then
                        draggingTrans = false
                    end
                end)
                AddConn(UserInputService.InputChanged:Connect(function(inp)
                    if inp.UserInputType == Enum.UserInputType.MouseMovement and draggingTrans then
                        local ay = TransparencySlider.AbsolutePosition.Y
                        local ah = TransparencySlider.AbsoluteSize.Y
                        local my = math.clamp(inp.Position.Y - ay, 0, ah)
                        Transparency = 1 - my / ah
                        Display()
                    end
                end))
            end

            local buttonY = Config.Transparency and 240 or 230
            local btnDone = New("TextButton", {
                Size = UDim2.fromOffset(100, 30),
                Position = UDim2.fromOffset(panelW - 120, buttonY),
                Text = "Done",
            ...
            local btnCancel = New("TextButton", {
                Size = UDim2.fromOffset(100, 30),
                Position = UDim2.fromOffset(panelW - 230, buttonY),
                Text = "Cancel",
...
function CoreUi:RefreshTheme()
    if TabContainer then
        TabContainer.BackgroundColor3 = T.TabContainerBg
        TabContainer.BackgroundTransparency = T.TabContainerBgT
    end
    for _, btn in next, NativeTabs do
        btn.BackgroundColor3 = T.NativeTabBg
        btn.BackgroundTransparency = T.NativeTabBgT
    end
    for _, btn in next, CustomTabButtons do
        btn.BackgroundColor3 = T.CustomTabBg
        btn.BackgroundTransparency = T.CustomTabBgT
    end
    for _, page in next, NativePages do
        page.BackgroundColor3 = T.NativePageBg
        page.BackgroundTransparency = T.NativePageBgT
    end
    for _, page in next, CustomTabPages do
        page.BackgroundColor3 = T.CustomPageBg
        page.BackgroundTransparency = T.CustomPageBgT
    end
end
...
CoreUi.InterfaceManager = {} do
    ...
    local Themes = {
        ...
        Light = {
            ...
            ScrollBarT = 0.7,
            NativePageBg = Color3.new(1,1,1),
            NativePageBgT = 0.95,
            CustomPageBg = Color3.new(1,1,1),
            CustomPageBgT = 0.95,
            NativeTabBg = Color3.new(1,1,1),
            NativeTabBgT = 0.9,
            CustomTabBg = Color3.new(1,1,1),
            CustomTabBgT = 0.9,
            TabContainerBg = Color3.new(1,1,1),
            TabContainerBgT = 0.95,
        },
        Midnight = {
            ...
            ScrollBarT = 0.5,
            NativePageBg = Color3.new(10,10,20),
            NativePageBgT = 0.98,
            CustomPageBg = Color3.new(10,10,20),
            CustomPageBgT = 0.98,
            NativeTabBg = Color3.new(10,10,20),
            NativeTabBgT = 0.95,
            CustomTabBg = Color3.new(10,10,20),
            CustomTabBgT = 0.95,
            TabContainerBg = Color3.new(10,10,20),
            TabContainerBgT = 0.98,
        },
    }
...
-- API pattern for VRHub integration
local Api = {}
...
-- VRHub registration if available
