...
local now = tick()
if Humanoid.FloorMaterial ~= Enum.Material.Air and (now - LastJump) > jumpCooldown then
if autoJumpType == "Realistic" then
do wait()
                Humanoid.Jump = true
game:GetService("Players").LocalPlayer.Character.Movement.JumpEnded:Fire()
end
else
...
local function spawnJumpEffect(character)
    if not character then return end
...
local function setupCharacter(CharacterInstance)
    JumpCap.Character = CharacterInstance
    JumpCap.Humanoid = CharacterInstance:FindFirstChildOfClass("Humanoid")
    JumpCap.JumpCount = 0
    JumpCap.ForceJumpDisabled = false
    JumpCap.LastJumpTime = 0
end
...
JumpCap.Connection = RunService.RenderStepped:Connect(function()
    if not JumpCap.Humanoid then return end
    
    if JumpCap.Humanoid.FloorMaterial ~= Enum.Material.Air then
        JumpCap.JumpCount = 0
        JumpCap.ForceJumpDisabled = false
    end
    
    if not JumpCap.ForceJumpDisabled then
        if JumpCap.Humanoid.Jump then
            if JumpCap.JumpCount < JumpCap.MaxJumps then
                if JumpCap.Humanoid.FloorMaterial == Enum.Material.Air and JumpCap.JumpCount > 0 then
                    spawnJumpEffect(JumpCap.Character)
                end
                
                JumpCap.Humanoid:ChangeState(Enum.HumanoidStateType.Jumping)
                JumpCap.JumpCount = JumpCap.JumpCount + 1
            end
        end
    end
    
    if JumpCap.Humanoid.Jump then
        JumpCap.ForceJumpDisabled = true
    else
        JumpCap.ForceJumpDisabled = false
    end
end)
...
Tabs.Teleport:Section({ Title = "Teleport to Coordinates", TextSize = 20 })
local teleInputValue = ""
Tabs.Teleport:Input({
...
Tabs.Teleport:Button({
    ...
    Icon = "target",
    Callback = function()
        local function parseCoordinate(token, current)
            ...
            if token:sub(1,1) == "~" then
                ...
                if rest == "" then return current end
                local op = rest:sub(1,1)
                local valuePart = rest:sub(2)
                if op == "+" or op == "-" or op == "*" then
                    local val = tonumber(valuePart)
                    if val then
                        if op == "+" then return current + val
                        elseif op == "-" then return current - val
                        elseif op == "*" then return current * val end
                    end
                else
                    local val = tonumber(rest)
                    if val then return current + val end
                end
            end
            return nil
        end
        ...
        local tokens = {}
        for token in string.gmatch(teleInputValue, "%S+") do
            table.insert(tokens, token)
        end
        if #tokens < 3 then
            WindUI:Notify({
                Title = "Invalid Input",
                Content = "Please enter three values for X, Y, Z separated by spaces",
                Duration = 3
            })
            return
        end
        ...
        local x = parseCoordinate(tokens[1], currentPos.X)
        local y = parseCoordinate(tokens[2], currentPos.Y)
        local z = parseCoordinate(tokens[3], currentPos.Z)

        if x == nil or y == nil or z == nil then
            WindUI:Notify({
                Title = "Invalid Expression",
                Content = "One or more coordinates could not be parsed",
                Duration = 3
            })
            return
        end
