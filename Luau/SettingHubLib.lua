...
local EspTargets = {}
local EspPlayerDropDown = nil

MakeSectionHeader(GamePage, "ESP", "ScanEye")

EspPlayerDropDown = MakeMultiDropDown(GamePage, "Target Players", {}, function(Selected)
        EspTargets = Selected
end)
...
Protect(function()
        local function RefreshEspPlayers()
                ...
                if EspPlayerDropDown then
                        EspPlayerDropDown:UpdateList(Names)
                end
        end
        Connect(Players.PlayerAdded, RefreshEspPlayers)
        Connect(Players.PlayerRemoving, function()
                task.defer(RefreshEspPlayers)
        end)
        RefreshEspPlayers()
end)

MakeSectionHeader(GamePage, "Visibility", "Eye")
MakeBooleanSelector(GamePage, "ESP", EspSettings, "Enabled")
MakeBooleanSelector(GamePage, "ESP Rainbow", EspSettings, "Rainbow")
MakeBooleanSelector(GamePage, "Highlights", EspSettings, "HighlightsEnabled")
MakeBooleanSelector(GamePage, "Rainbow Highlights", EspSettings, "HighlightsRainbow")

MakeSectionHeader(GamePage, "ESP Details", "ScanEye")
MakeBooleanSelector(GamePage, "ESP Boxes", EspSettings, "Boxes")
MakeBooleanSelector(GamePage, "ESP Names", EspSettings, "Names")
MakeBooleanSelector(GamePage, "Health Bar", EspSettings, "HealthBar")
MakeBooleanSelector(GamePage, "Distance", EspSettings, "Distance")
MakeBooleanSelector(GamePage, "Tracers", EspSettings, "Tracers")
MakeBooleanSelector(GamePage, "Chams", EspSettings, "Chams")
...
SetVisibility = function(Visible, NoAnimation, CustomPage)
        local Now = tick()
        if Visible and (Now - LastVisibilityChange) < 2 and not NoAnimation then return end
        if not Visible then
                SetGameplayInputLocked(false)
        end
        if Hub.Visible == Visible and not CustomPage then return end
        if Visible then LastVisibilityChange = Now end
        Hub.Visible = Visible
        ...
        if Visible then
                ...
                if NoAnimation then
                        ...
                        TweenTo(Hub.Shield, SettingsActivePosition, Enum.EasingDirection.InOut, Enum.EasingStyle.Quart, 0.5)
                end
                ...
                if NoAnimation then
                        ...
                        TweenTo(Hub.Shield, SettingsInactivePosition, Enum.EasingDirection.In, Enum.EasingStyle.Quad, 0.4, function()
