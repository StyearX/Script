...
for i, c in next, TabContainer:GetChildren() do
    if c:IsA("TextButton") then
        table.insert(NativeTabs, c)
        c.LayoutOrder = i
        local tabLabel = c:FindFirstChild("TabLabel")
        if tabLabel then
            local title = tabLabel:FindFirstChild("Title")
            if title then title.TextTransparency = TAB_INACTIVE_TRANS end
            local icon = tabLabel:FindFirstChild("Icon")
            if icon then
                if icon:IsA("ImageLabel") then icon.ImageTransparency = TAB_INACTIVE_TRANS
                elseif icon:IsA("TextLabel") then icon.TextTransparency = TAB_INACTIVE_TRANS end
            end
        end
    end
end

local function OnNativeTabClick(activeNative)
    ShowNativePages()
...
local NativePageStructural = { InnerCanvasGroupShow = true, InnerCanvasGroupHide = true, UIPadding = true, UIListLayout = true }
local NativePagePositions = {}

local function SaveNativePagePositions()
    for _, child in next, PageInner:GetChildren() do
        if not NativePageStructural[child.Name] and not child.Name:find("^CoreUi_") then
            if child:IsA("Frame") or child:IsA("ScrollingFrame") or child:IsA("ImageButton") then
                NativePagePositions[child] = child.Position
            end
        end
    end
end
...
local function HideNativePages()
    for child, _ in next, NativePagePositions do
        pcall(function() child.Position = UDim2.new(2, 0, 0, 0) end)
    end
    local showGroup = PageInner:FindFirstChild("InnerCanvasGroupShow")
    local hideGroup = PageInner:FindFirstChild("InnerCanvasGroupHide")
    if showGroup then showGroup.Visible = false end
    if hideGroup then hideGroup.Visible = false end
end

local function ShowNativePages()
    for child, origPos in next, NativePagePositions do
        pcall(function() child.Position = origPos end)
    end
    local showGroup = PageInner:FindFirstChild("InnerCanvasGroupShow")
    if showGroup then showGroup.Visible = true end
end

task.defer(SaveNativePagePositions)
