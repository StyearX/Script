...
local function MakeStyledButton(name, text, size, clicked)
	local button = New("ImageButton", {
		Name = name, Image = "rbxasset://textures/ui/Settings/MenuBarAssets/MenuButton.png",
		ScaleType = Enum.ScaleType.Slice, SliceCenter = Rect.new(8,6,46,44),
		ImageColor3 = Color3.fromRGB(10,10,10), ImageTransparency = 0.35,
		AutoButtonColor = false, BackgroundTransparency = 1, Size = size, ZIndex = 6,
	})
	local label = New("TextLabel", {
		Name = name.."TextLabel", Parent = button, BackgroundTransparency = 1, BorderSizePixel = 0,
		Size = UDim2.new(1,0,1,-8), Position = UDim2.new(0,0,0,0),
		Font = Enum.Font.SourceSansBold, TextSize = 24, TextColor3 = Color3.new(1,1,1),
		TextXAlignment = Enum.TextXAlignment.Center, TextYAlignment = Enum.TextYAlignment.Center,
		Text = text, TextWrapped = true, ZIndex = 7,
	})
	local function onEnter() button.Image = "rbxasset://textures/ui/Settings/MenuBarAssets/MenuButtonSelected.png"; button.ImageColor3 = Color3.fromRGB(10,10,10); button.ImageTransparency = 0.2 end
	local function onLeave() button.Image = "rbxasset://textures/ui/Settings/MenuBarAssets/MenuButton.png"; button.ImageColor3 = Color3.fromRGB(10,10,10); button.ImageTransparency = 0.35 end
	AddConn(button.MouseEnter:Connect(onEnter))
	AddConn(button.MouseLeave:Connect(onLeave))
	if clicked then AddConn(button.MouseButton1Click:Connect(clicked)) end
	return button, label
end

local function TabMethods(tab)
	tab._page = tab._page or tab.Page
	tab.AddRow = function(self, name, height)
		local row = New("ImageButton", {
			Name = name.."Row", Parent = self._page,
			BackgroundTransparency = 1, BorderSizePixel = 0, Image = "",
			Active = false, AutoButtonColor = false, Selectable = false,
			Size = UDim2.new(1,0,0,height or 50), ZIndex = 5,
		})
		New("TextLabel", {
			Name = name.."Label", Parent = row, BackgroundTransparency = 1,
			Font = Enum.Font.SourceSansBold, TextSize = 24, TextColor3 = Color3.new(1,1,1),
			TextXAlignment = Enum.TextXAlignment.Left, Text = name,
			Size = UDim2.new(0,200,1,0), Position = UDim2.new(0,10,0,0), ZIndex = 6,
		})
		return row
	end
	...
	tab.AddSlider = function(self, name, steps, index, changed, minStep)
		minStep = minStep or 0
		local currentIndex = math.clamp(index or 1, minStep, steps)
		local row = self:AddRow(name)
		local holder = New("Frame", { Parent = row, BackgroundTransparency = 1, Size = UDim2.new(0,502,0,50), Position = UDim2.new(1,-502,0.5,-25), Active = true, ZIndex = 6 })
		local left = New("ImageButton", { Parent = holder, BackgroundTransparency = 1, Image = "", Size = UDim2.new(0,60,0,50), Position = UDim2.new(0,-10,0.5,-25), ZIndex = 7 })
		...
		local right = New("ImageButton", { Parent = holder, BackgroundTransparency = 1, Image = "", Size = UDim2.new(0,50,0,50), Position = UDim2.new(1,-50,0.5,-25), ZIndex = 7 })
		...
		local function refresh(immediate)
			for i, seg in next, segments do
				local selected = api.Interactable and i <= currentIndex
				local color = selected and Color3.fromRGB(0,162,255) or Color3.fromRGB(15,15,15)
				if i == 1 or i == steps then
					...
					seg.ImageTransparency = 0.36; seg.BackgroundTransparency = 1
				else
		...
		local function setSliderValue(newIndex)
			...
			currentIndex = newIndex; refresh(); if changed then changed(currentIndex) end
		end
		...
		local function setSliderFromX(x)
			...
			local startX = first.AbsolutePosition.X; local endX = last.AbsolutePosition.X + last.AbsoluteSize.X
			local alpha = math.clamp((x - startX) / (endX - startX), 0, 1)
			if minStep > 0 then setSliderValue(math.clamp(math.floor((alpha * steps) + 1), minStep, steps))
			else setSliderValue(math.clamp(math.floor(alpha * (steps + 1)), 0, steps)) end
		end

		for i = 1, steps do
			local seg = New("ImageButton", {
				Parent = holder, BackgroundColor3 = Color3.fromRGB(15,15,15), BackgroundTransparency = 0.36,
				BorderSizePixel = 0, AutoButtonColor = false, Image = "", ImageTransparency = 0.36,
				Size = UDim2.new(0,35,0,25), Position = UDim2.new(0,60+((i-1)*39),0.5,-12), ZIndex = 7,
			})
			if i == 1 or i == steps then seg.BackgroundTransparency = 1; seg.ScaleType = Enum.ScaleType.Slice; seg.SliceCenter = Rect.new(3,3,32,21) end
			segments[i] = seg
			AddConn(seg.MouseButton1Click:Connect(function() if api.Interactable then setSliderValue(i) end end))
		end

		local capture = New("TextButton", { Parent = holder, BackgroundTransparency = 1, BorderSizePixel = 0, Text = "", AutoButtonColor = false, Active = true, Size = UDim2.new(0,400,1,0), Position = UDim2.new(0,52,0,0), ZIndex = 9 })
		AddConn(capture.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then dragging = true; setSliderFromX(input.Position.X) end end))
		AddConn(capture.InputChanged:Connect(function(input) if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then setSliderFromX(input.Position.X) end end))
		AddConn(UserInputService.InputChanged:Connect(function(input) if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then setSliderFromX(input.Position.X) end end))
		AddConn(UserInputService.InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then dragging = false end end))
		...
		function api:SetValue(value) currentIndex = math.clamp(value, minStep, steps); refresh(true) end
		function api:GetValue() return currentIndex end
		function api:SetInteractable(interactable)
			api.Interactable = interactable; holder.Active = interactable; holder.ZIndex = interactable and 6 or 5
			for _, seg in next, segments do seg.Active = interactable; seg.Selectable = interactable; seg.ZIndex = interactable and 7 or 5 end
			refresh(true)
		end
		return api
	end

	tab.AddSelector = function(self, name, values, index, changed)
		local currentIndex = index or 1
		local row = self:AddRow(name)
		local selectorFrame = New("ImageButton", { Parent = row, BackgroundTransparency = 1, Image = "", AutoButtonColor = false, Size = UDim2.new(0,502,0,50), Position = UDim2.new(1,-502,0.5,-25), ZIndex = 6 })
		local left = New("ImageButton", { Parent = selectorFrame, BackgroundTransparency = 1, Image = "", Size = UDim2.new(0,60,0,50), Position = UDim2.new(0,-10,0.5,-25), ZIndex = 7 })
		New("ImageLabel", { Parent = left, BackgroundTransparency = 1, Image = "rbxasset://textures/ui/Settings/Slider/Left.png", Size = UDim2.new(0,18,0,30), Position = UDim2.new(1,-24,0.5,-15), ZIndex = 8 })
		local right = New("ImageButton", { Parent = selectorFrame, BackgroundTransparency = 1, Image = "", Size = UDim2.new(0,50,0,50), Position = UDim2.new(1,-50,0.5,-25), ZIndex = 7 })
		New("ImageLabel", { Parent = right, BackgroundTransparency = 1, Image = "rbxasset://textures/ui/Settings/Slider/Right.png", Size = UDim2.new(0,18,0,30), Position = UDim2.new(0,6,0.5,-15), ZIndex = 8 })
		local label = New("TextLabel", { Parent = selectorFrame, Name = "Selection", BackgroundTransparency = 1, BorderSizePixel = 0, Size = UDim2.new(1,-120,1,0), Position = UDim2.new(0,60,0,0), TextColor3 = Color3.new(1,1,1), TextTransparency = 0.2, TextYAlignment = Enum.TextYAlignment.Center, Font = Enum.Font.SourceSans, TextSize = 24, Text = values[currentIndex], ZIndex = 7 })
		local api = { CurrentIndex = currentIndex, Interactable = true, RowFrame = row, Selection = selectorFrame, TextLabel = label }
		local function setText(text, dir) label.Text = text; label.Position = UDim2.new(0,60 + ((dir or 0)*16),0,0); label.TextTransparency = 0.75; Tween(label, 0.15, { Position = UDim2.new(0,60,0,0) }); FadeText(label, 0.2) end
		local function apply(delta)
			...
			api.CurrentIndex = currentIndex; setText(values[currentIndex], delta); if changed then changed(currentIndex, values[currentIndex]) end
		end
		...
		function api:SetSelectionIndex(newIndex, fireChanged)
			...
			currentIndex = math.clamp(newIndex, 1, #values); api.CurrentIndex = currentIndex; setText(values[currentIndex], 0)
			if changed and fireChanged then changed(currentIndex, values[currentIndex]) end
		end
		function api:SetInteractable(interactable)
			api.Interactable = interactable; selectorFrame.ImageTransparency = interactable and 0 or 0.65; label.TextTransparency = interactable and 0.2 or 0.65; left.Visible = interactable; right.Visible = interactable
		end
	...
	tab.AddDropDown = function(self, name, values, index, changed)
		local currentIndex = index
		local row = self:AddRow(name)
		local btn, lbl = MakeStyledButton(name.."DropDown", values[currentIndex] or "Choose One", UDim2.new(0,300,0,44))
		btn.Parent = row; btn.Position = UDim2.new(1,-350,0.5,-22)
		local arrow = New("ImageLabel", { Parent = btn, BackgroundTransparency = 1, Image = "rbxasset://textures/ui/Settings/DropDown/DropDown.png", Size = UDim2.new(0,15,0,10), Position = UDim2.new(1,-40,0.5,-7), ZIndex = 8 })
		local api = { CurrentIndex = currentIndex, Interactable = true }
		local overlay = New("TextButton", { Parent = OverlayRoot, Name = name.."DropDownFullscreenFrame", Visible = false, BackgroundColor3 = Color3.new(0,0,0), BackgroundTransparency = 0.2, BorderSizePixel = 0, Text = "", Size = UDim2.new(1,0,1,0), ZIndex = 20 })
		local panel = New("ImageLabel", { Parent = overlay, Image = "rbxasset://textures/ui/Settings/MenuBarAssets/MenuButton.png", ScaleType = Enum.ScaleType.Slice, SliceCenter = Rect.new(8,6,46,44), BackgroundTransparency = 1, Size = UDim2.new(0,400,0.9,0), Position = UDim2.new(0.5,-200,0.05,0), ZIndex = 21 })
		local list = New("ScrollingFrame", { Parent = panel, BackgroundTransparency = 1, BorderSizePixel = 0, Size = UDim2.new(1,-20,1,-25), Position = UDim2.new(0,10,0,10), CanvasSize = UDim2.new(0,0,0,#values*51), ScrollBarThickness = 6, ZIndex = 21 })
		local function rebuild(newValues)
			...
			for i, val in next, values do
				local opt = New("TextButton", { Parent = list, Name = "Selection"..tostring(i), BackgroundTransparency = 1, BorderSizePixel = 0, AutoButtonColor = false, Size = UDim2.new(1,-28,0,50), Position = UDim2.new(0,14,0,(i-1)*51), TextColor3 = (i == currentIndex and Color3.new(1,1,1)) or Color3.new(0.7,0.7,0.7), Font = Enum.Font.SourceSans, TextSize = 24, Text = val, ZIndex = 22 })
				AddConn(opt.MouseButton1Click:Connect(function() api:SetSelectionIndex(i, true); overlay.Visible = false end))
			end
			list.CanvasSize = UDim2.new(0,0,0,#values*51); api:SetSelectionIndex(currentIndex, false)
		end
		AddConn(btn.MouseButton1Click:Connect(function() if api.Interactable then overlay.Visible = true end end))
		...
		function api:SetSelectionIndex(newIndex, fireChanged)
			if not newIndex or newIndex < 1 or newIndex > #values then currentIndex = nil; api.CurrentIndex = nil; lbl.Text = "Choose One"; if changed and fireChanged then changed(nil, nil) end; return false end
			currentIndex = newIndex; api.CurrentIndex = currentIndex; lbl.Text = values[currentIndex]; if changed and fireChanged then changed(currentIndex, values[currentIndex]) end; return true
		end
		function api:GetSelectedIndex() return currentIndex end
		function api:GetSelectedValue() return currentIndex and values[currentIndex] or nil end
		function api:SetInteractable(interactable)
			api.Interactable = interactable; btn.ImageTransparency = interactable and 0 or 0.65; btn.Active = interactable; btn.Selectable = interactable; if lbl then lbl.TextTransparency = interactable and 0 or 0.65 end; if arrow then arrow.ImageTransparency = interactable and 0 or 0.65 end
		end
		return api
	end
