...
-- === Original UI Components from settings (2).lua ===
...
local function MakeSlider(page, name, steps, index, changed, minStep)
	...
	local function refresh(immediate)
		for i, seg in next, segments do
			...
			-- Change colors: filled = white, empty = gray
...
local function MakeSelector(page, name, values, index, changed)
	...
	local api = { CurrentIndex = currentIndex, Interactable = true, Selection = selectorFrame, TextLabel = label }
...
local function MakeDropDown(page, name, values, index, changed)
	...
	local function rebuild(newValues)
		...
		api:SetSelectionIndex(currentIndex, false)
	end
...
function CoreUi:Tab(section, config)
	...
	function tab:AddToggle(config)
		...
		-- reposition selector to right side
		selector.Selection.Position = UDim2.new(1, -502, 0.5, -25)
		selector.Selection.Size = UDim2.new(0, 200, 0, 44)
		-- adjust left/right arrows visibility
		return selector
	end
	...
	function tab:AddSlider(config)
		...
		local startIndex = math.clamp(math.floor((default - min) / step) + 1, 1, steps)
		...
		local slider = MakeSlider(self, title.."_slider", steps, startIndex, function(value)
			local realValue = min + (value - 1) * step
			callback(realValue)
		end, 1)
		return slider
	end
	...
	function tab:AddDropdown(config)
		...
		-- Convert options to simple list of strings for MakeDropDown
		local optLabels = {}
		local optValues = {}
		...
		dropdown.DropDownFrame.Position = UDim2.new(1, -350, 0.5, -22)
		dropdown.DropDownFrame.Size = UDim2.new(0, 300, 0, 44)
		return dropdown
	end
