...
local function sendHubLoadedMessage()
	local message = " Owner Version | The Worst loaded. "
	local sent = false
	pcall(function()
...
TargetGroup:AddToggle("LoopKickGrabToggle", { -- ?????????? ID
	Text = "??? ?????",
	Default = false,
	Callback = function(on)
		kickLoopEnabled = on
...
TargetGroup:AddToggle("RagdollSnowballKick", { -- ?????????? ID
	Text = "??? ?????",
	Default = false,
	Callback = function(on)
		local Players = game:GetService("Players")
               ...
               -- ===============================
				pcall(function()
					local offset = Vector3.new(
                       math.random(-30, 30) / 100,  -- ±0.3 ?? X
                       math.random(-30, 30) / 100,  -- ±0.3 ?? Y
                       math.random(-30, 30) / 100   -- ±0.3 ?? Z
                   )
...
ThemeManager:SetFolder("The Worst")
SaveManager:SetFolder("The Worst/Configs")
...
-- ===============================
-- KICK AURA 1 HAND (grab + blob) ??? AurasGroup
-- ===============================
